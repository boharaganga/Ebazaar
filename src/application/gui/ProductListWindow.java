 ...
 *        to the selected catalog group. The catalog group is passed into the
 *        constructor as a parameter. In the final version of the application,
 *        the products displayed should be all products in the database having
 *        category group equal to this parameter.
 *        <p>
 *        <table border="1">
 *        <tr>
 ...
 * 
 */
public class ProductListWindow extends javax.swing.JWindow implements
		ParentWindow {
	private BrowseAndSelectController control;
	private Logger logger = Logger.getLogger(ProductListWindow.class.getName());
	// constants
	 ...
	 * 
	 * @param type
	 *            - the category group (either books or clothes)
	 */
	...
	private void updateModel() {
		...
		if (USE_DEFAULT_DATA) {
			...
			try {
				List<IProductFromDb> products = facade
						.getProductList(catalogType);
				for (IProductFromDb product : products) {
					theData.add(new String[] { product.getProductName(),
							product.getUnitPrice(), product.getMfgDate(),
							product.getQuantityAvail() });
				}
			} catch (DatabaseException e) {
				JOptionPane.showMessageDialog(this,
						"Unable to get product by catalog", "Error",
	...
	private void defineLowerPanel() {
		...
		selectButton.addActionListener(control.getSelectProductListener(this));

		// back button
		JButton backButton = new JButton(BACK);
		backButton
				.addActionListener(control.getBackToCatalogListListener(this));
